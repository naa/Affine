...
Subalgebra  $\bff_{+}\subset \gf$ spanned by the generators $H^{i}, E^{\alpha}$ for positive roots $\alpha\in \Delta^{+}$ is called Borel subalgebra.
...
Consider an action of element $s_{\alpha}s_{\alpha+\delta}$ of Weyl group of affine Lie algebra  $\hat\gf$ for $\alpha$ simple root of underlying finite-dimensional Lie algebra $\gf$. Using the definition \eqref{eq:8} it is easy to see that $s_{\alpha}s_{\alpha+\delta} \cdot \lambda=\lambda+\frac{2}{(\alpha,\alpha)}\alpha+\left(\frac{(\alpha,\alpha)}{2 (\lambda,\delta)}+(\lambda,\alpha)\right) \delta$. So Weyl group can be presented as semidirect product of Weyl group $W_{\gf}$ of $\gf$ and translations corresponding to the roots of $\gf$. 

Weyl group element can be presented as the product of elementary reflections in multiple ways. Number of elementary reflections in shortest sequence representing element $w\in W_{\gf}$ is called {\it length} of $w$ and denoted $l(w)$. We also use the notation $\epsilon(w)=(-1)^{l(w)}$ for parity of number of Weyl reflections generating $w$. 
...
\end{equation}
For finite-dimensional Lie algebra there are $r$ fundamental weights, $i=1,\dots, r$. For affine Lie algebra we have additional fundamental weight $\omega_0=\lambda$, $(\lambda,\delta)=1, \; (\lambda,\lambda)=(\delta,\delta)=0$. Other fundamental weights are equal to $\omega_i=a_i^v \lambda_0 +\co{\omega_i}$, where $\co{\omega_i}$ is the fundamental weight of finite-dimensional Lie algebra $\gf$.

{\it Weyl vector} is equal to the sum of fundamental weights $\rho=\sum_{i} \omega_{i}$. It is of importance for the construction of algebra modules.
...
where $\underset{U(\bff_{+})}{\otimes}$ means that the action of elements of $U(\bff_{+})$ ``falls through'' the left part of tensor product onto the right part. Here $\bff_{+}$ is Borel subalgebra, $D^{\mu}(\bff_{+})$ is a representation of $\bff_{+}$ such that $D(E^{\alpha})=0,\; D(H)=\mu(H)$ for any positive root $\alpha$.
...
\end{equation}
Formulae for Verma and generalized Verma modules differs only in second term on the right-hand side. In case of Verma module it is just $\delta_{\xi,\mu}$. For generalized Verma module summation in second term on the right-hand side \eqref{eq:14} is over the Weyl subgroup generated by reflections corresponding to roots $\{\alpha_{I}\}$.
...
Note that it is applicable only to irreducible only to irreducible modules. 
...
It is possible to calculate branching coefficients by the construction and repeated subtraction of characters of modules $L^{\nu}_{\af}$. This traditional approach has serious limitations especially in case of affine Lie algebras. We discuss them in the end of section \ref{sec:comp-algor}. 

Now we describe alternative approach which is based upon recurrent relation on branching coefficients. 
But before we proceed to this recurrent relation we need several definitions.
...
Cartan subalgebra can be decomposed in the following way:  $\frak{h}=\frak{\frak{h}_{\af}}\oplus
...
$\gf\subset \gf\otimes \gf$ (see \cite{LyakhovskyPostnova2011}). So our implementation of recurrent algorithm can be used to decompose tensor products (See Section \ref{sec:tens-prod-decomp}).
...
coordinates of the weight in the orthogonal basis of Bourbaki 
...
Elements of Weyl group are specified by the set of indices of reflections, so element  $w=s_{1}s_{2}s_{1}$ of Weyl group of algebra $B_{2}$ is constructed with \lstinline{weylGroupElement[b2][1,2,1]}. Then it can be applied to the weights:
\begin{lstlisting}
...
\end{enumerate}
The difference in performance of algorithms is in the number of previous values required to get the multiplicity of weight under consideration. For  Weyl formula-based recurrent relation \eqref{eq:14} it is constant and equal to the number of elements in Weyl group (if we are far from the boundary of representation diagram). When Freudenthal formula \eqref{eq:15} is used number of previous values grows with the distance from the external border of representation. So Freudenthal formula is faster if the weight is close to the border or the rank of the algebra and the size of Weyl group is large \cite{moody1982fast}.
...
\begin{figure}[h]
  ...
  \caption{Running time of algorithms based on Freudenthal formula \eqref{eq:15} (dashed) and recurrent relation \eqref{eq:14} (solid) with the number of weights in $\bar C$ for calculation of multiplicities in representations of $B_{2}$.}
...
\begin{figure}[h]
  ...
  \caption{Running time of algorithms based on Freudenthal formula \eqref{eq:15} (dashed) and recurrent relation \eqref{recurrent-rel} (solid) with the number of weights in $\bar C$ for calculation of branching coefficients for $B_{3}\subset B_{4}$.}
  \label{fig:branching}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Character of $G_{2}$ generalized Verma modules appearing in decomposition of $L^{[1,1]}$. Parabolic Verma modules in upper row appear in decomposition with positive sign, in lower row -- with negative.}
  \label{g2-pverma}
\end{figure}
...
\end{lstlisting}
Similarly for affine Lie algebra $\hat G_{2}$ we get
\begin{lstlisting}[mathescape=true]
...
In future versions of our software we are going to treat twisted affine Lie algebras, extended affine Lie algebras and provide more direct support for tensor product decomposition. 
